...
interface TransacaoDao {
    ...
    @Query("SELECT * FROM transacoes WHERE tenantId = :tenantId AND deleted = 0 ORDER BY date DESC")
    fun observeAll(tenantId: Long): Flow<List<TransacaoEntity>>

    @Query("SELECT * FROM transacoes WHERE id = :id LIMIT 1")
    suspend fun getById(id: Long): TransacaoEntity?

    @Query("SELECT * FROM transacoes WHERE patientId = :patientId AND tenantId = :tenantId AND date BETWEEN :start AND :end ORDER BY date DESC")
    fun observeByPatientAndRange(patientId: Long, tenantId: Long, start: String, end: String): Flow<List<TransacaoEntity>>

    @Query("""SELECT COALESCE(SUM(amountBrl),0.0) FROM transacoes 
              WHERE status = :status AND tenantId = :tenantId AND date BETWEEN :start AND :end AND type = :type""")
    suspend fun sumByStatusAndRange(status: String, tenantId: Long, start: String, end: String, type: String): Double

    @Query("""SELECT COALESCE(SUM(amountBrl),0.0) FROM transacoes 
              WHERE tenantId = :tenantId AND date BETWEEN :start AND :end""")
    suspend fun sumRevenue(tenantId: Long, start: String, end: String): Double

    @Query("""SELECT COALESCE(SUM(amountBrl),0.0) FROM transacoes 
              WHERE status = 'PAGO' AND tenantId = :tenantId AND date BETWEEN :start AND :end""")
    suspend fun sumPaid(tenantId: Long, start: String, end: String): Double

    @Query("""SELECT COALESCE(SUM(amountBrl),0.0) FROM transacoes 
              WHERE status = 'PENDENTE' AND tenantId = :tenantId AND date BETWEEN :start AND :end""")
    suspend fun sumPending(tenantId: Long, start: String, end: String): Double

    @Query("""SELECT * FROM transacoes WHERE tenantId = :tenantId AND pendingSync = 1 AND deleted = 0 
              AND lastModified >= :since ORDER BY lastModified ASC""")
    suspend fun getChangedSince(tenantId: Long, since: String): List<TransacaoEntity>
    ...
    @Query("SELECT * FROM transacoes WHERE tenantId = :tenantId AND pendingSync = 1 ORDER BY lastModified ASC")
    suspend fun getPendingSync(tenantId: Long): List<TransacaoEntity>
