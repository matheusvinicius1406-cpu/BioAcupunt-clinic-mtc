...
class AppointmentRepositoryImpl(
    ...
    override fun observeByDate(date: String): Flow<List<Appointment>> {
        return dao.observeByDate(date).map { it.map { e -> e.toDomain() } }.catch { emit(emptyList()) }
    }

    override fun observeByPatient(patientId: Long): Flow<List<Appointment>> {
        return dao.observeByPatient(patientId).map { it.map { e -> e.toDomain() } }.catch { emit(emptyList()) }
    }

    override fun observeByStatus(status: String): Flow<List<Appointment>> {
        return dao.observeByStatus(status).map { it.map { e -> e.toDomain() } }.catch { emit(emptyList()) }
    }

    override fun observeBetween(start: String, end: String): Flow<List<Appointment>> {
        return dao.observeBetween(start, end).map { it.map { e -> e.toDomain() } }.catch { emit(emptyList()) }
    }
    ...
    override suspend fun getByDateSync(date: String): List<Appointment> {
        return try { dao.getByDateSync(date).map { it.toDomain() } } catch (e: Exception) { emptyList() }
    }
    ...
    override suspend fun countByDate(date: String): Result<Int> {
        return try { Result.Success(dao.countByDate(date)) } catch (e: Exception) { Result.Error(AppError.DatabaseError(e)) }
    }

    override suspend fun countByStatus(status: String): Result<Int> {
        return try { Result.Success(dao.countByStatus(status)) } catch (e: Exception) { Result.Error(AppError.DatabaseError(e)) }
    }
}
