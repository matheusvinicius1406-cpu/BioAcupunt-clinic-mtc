...
| C-05 | Workflow needs billing adaptation | CONFIRMED | 443 runtime imports, 140 engine deps | **STRENGTHENED** — Workflow is deeply coupled, not just billing |
| C-06 | Schema-per-tenant is optimal | UNPROVEN | Need sensitivity analysis | **PENDING** |
| C-07 | Person is canonical identity | CONFIRMED | Entity-only, no business logic | **CONFIRMED** |
...
| N-04 | Workflow has massive runtime (391 files, 1370 runtime imports) | Import analysis | **PROVEN** |
| N-05 | Dashboard has large runtime (154 files, 452 runtime imports) | Import analysis | **PROVEN** |
| N-06 | Auth has 158 files, 15 SSO-related files | File analysis | **PROVEN** |
...
2. **Workflow is dominant** — 1370 runtime imports, deeply coupled to engine
3. **Dashboard is large** — 452 runtime imports
4. **Auth has SSO coupling** — 71 SSO runtime imports, 15 SSO files
